...
struct myargs my_options[] = {
    ...
    {"mac-addresses", 'M', "unique MAC/IP addresses per switch", MYARGS_INTEGER, {.integer = 1000}},
...
int main(int argc, char * argv[])
{
    ...
    int     total_ip_addresses = myargs_get_default_integer(my_options, "mac-addresses");
    ...
    while(1)
    {
        ...
        switch (c) 
        {
            ...
            case 'M':
                ...
                total_ip_addresses = atoi(optarg);
                break;
            case 'm': 
    ...
    for( i = 0; i < n_fakeswitches; i++)
    {
        ...
        #else
        fakeswitch_init(&fakeswitches[i], 0, sock, 65536, debug, delay, mode, total_mac_addresses, total_ip_addresses, learn_dst_macs);
        #endif
