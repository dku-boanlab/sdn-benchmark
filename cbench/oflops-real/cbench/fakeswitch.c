...
static int make_packet_in(int switch_id, int xid, int buffer_id, char * buf, int buflen, int mac_address, int ip_address);
...
void fakeswitch_init(struct fakeswitch *fs, int dpid, int sock, int bufsize, int debug, int delay, enum test_mode mode, int total_mac_addresses, int total_ip_addresses, int learn_dstmac)
{
    ...
    fs->probe_state = 0;
    fs->mode = mode;
    fs->probe_size = make_packet_in(fs->id, 0, 0, buf, BUFLEN, fs->current_mac_address++, fs->current_ip_address++);
    fs->count = 0;
    fs->switch_status = START;
    fs->delay = delay;
    fs->total_mac_addresses = total_mac_addresses;
    fs->total_ip_addresses = total_ip_addresses;
    fs->current_mac_address = 0;
    fs->current_ip_address = 0;
    fs->xid = 1;
...
static int make_packet_in(int switch_id, int xid, int buffer_id, char * buf, int buflen, int mac_address, int ip_address)
{
    ...
    const char fake[] = {
		...
		0xc0,0xa8,0x00,0x00,0xc0,0xa8,0x01,0x28,0x7a,0x18,0x58,
    ...
    ip = (struct iphdr *) ((uint8_t *)eth + sizeof(struct ether_header));
    ip->saddr = ip->saddr | (htons(ip_address) << 16) | switch_id;
    if (((uint8_t)((ip->saddr) >> 24)) == 0 || (ip->saddr == ip->daddr)) {
        mac_address++;
        memcpy(&eth->ether_shost[1], &mac_address, sizeof(mac_address));
        eth->ether_dhost[5] = switch_id;
        eth->ether_shost[5] = switch_id;
...
static void fakeswitch_handle_write(struct fakeswitch *fs)
{
    ...
    if( fs->switch_status == READY_TO_SEND) 
    {
        ...
        for (i = 0; i < send_count; i++)
        {
            ...
            count = make_packet_in(fs->id, fs->xid++, fs->current_buffer_id, buf, BUFLEN, fs->current_mac_address, fs->current_ip_address);
            fs->current_mac_address = ( fs->current_mac_address + 1 ) % fs->total_mac_addresses;
            fs->current_ip_address = ( fs->current_ip_address + 1 ) % fs->total_ip_addresses;
